...
*   **Fact-Based Introduction Generation:**
    *   Generates a concise introductory summary (Banner) that highlights a candidate's qualifications as they relate to a specific job description.
    *   This process is strictly sequential: it occurs *after* the experience section has been refined and the full resume has been reconstructed with the latest content.
    *   The LLM analyzes the entire reconstructed resume (including experience, education, projects, certifications, and the original user-supplied banner) to find factual evidence supporting the candidate's fit for the role.
    *   The generated introduction is based solely on this factual evidence to prevent fabrication or exaggeration.
    *   In case of generation failure, a placeholder message is used.

*   **Experience Section Refinement:**
    *   Refines the "Experience" section of a resume to align with a job description.
    *   Analyzes job descriptions with an LLM to extract key skills and qualifications.
    *   Performs a multi-pass refinement where each role is individually refined against the job analysis.

*   **Common Features:**
    *   Provides real-time progress feedback for all AI tasks using Server-Sent Events (SSE), with graceful error handling.
    *   Allows users to configure custom OpenAI-compatible API endpoints and model names.
